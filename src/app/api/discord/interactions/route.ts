...
// Helper to send follow-up message after deferred response
async function sendFollowUp(applicationId: string, token: string, content: string) {
    ...
    if (!response.ok) {
        console.error('[Discord] Follow-up failed:', await response.text());
    }
}

// Process level command
async function processLevelCommand(levelNum: number, gameName: string | null): Promise<string> {
    ...
    if (listError || !files) {
        return `Error listing data: ${listError?.message || 'No files found'}`;
    }
    ...
    if (!matchingFile) {
        return `No Level Revize data found${gameNameToMatch ? ` for game '${gameNameToMatch}'` : ''}. Please load data from the Weekly Check page first.`;
    }
    ...
    if (downloadError || !fileData) {
        return `Error downloading data: ${downloadError?.message || 'Unknown error'}`;
    }
    ...
    if (filteredRows.length === 0) {
        return `No data found for level ${levelNum} (+/- 5)${gameNameToMatch ? ` in game '${gameNameToMatch}'` : ''}.`;
    }
    ...
    return `**Level Context: ${levelNum} (${extractedGameName})**\n\`\`\`\n${header}\n${tableRows.join('\n')}\n\`\`\``;
}
...
export async function POST(request: Request) {
    ...
    if (body.type === InteractionType.APPLICATION_COMMAND) {
        ...
        if (name === 'level') {
            ...
            const gameName = options?.find((o: any) => o.name === 'game')?.value || null;
